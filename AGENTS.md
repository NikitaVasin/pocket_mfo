...
При интеграции репозитория в приложение прочитайте [единое руководство для AI-агента](docs/AI_INTEGRATION.md). До изменений обязательно сопоставьте целевой проект с [критериями совместимости v1.0.0](docs/COMPATIBILITY.md) и запишите результаты/расхождения в integration-plan. Руководство задаёт обязательное интервью, запрос необходимых ключей/файлов, порядок подключения и проверок, а также передачу пользователю дальнейших действий в кабинетах внешних служб. При обычной локальной правке повторять всё интеграционное интервью не требуется.
...
Публичный репозиторий: `github.com/NikitaVasin/pocket_mfo`, тот же путь используется в go.mod и импортах. Не меняйте регистр имени владельца. Первый выпуск каталога — **v1.0.0** (VERSION: 1.0.0), выбран владельцем. Не передвигайте существующий тег и не назначайте следующие версии без запроса. Пользователь попросил пока не добавлять лицензию. Подготовка файлов не означает публикацию или разрешение отправлять изменения в GitHub. Порядок дальнейших действий — [docs/PUBLISHING.md](docs/PUBLISHING.md).
