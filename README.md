...
Для плагина Variants создаются `demo_members` (6 пользователей), `demo_subscriptions` (3 подписки) и `demo_offers` (12 записей в 6 наборах). Настройки доступны во вкладке **Variants** коллекции `demo_offers`; выбор набора — в поле `content_set` записи. Общий пароль демопользователей: **`demo-variants-123`**.
