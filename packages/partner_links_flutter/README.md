...
PocketBase должен быть авторизован обычным пользователем разрешённой сервером auth-коллекции. В настройках плагина укажите текстовое поле пользователя с profileId, совпадающим с AppMetrica SDK. Сервер читает его из записи авторизованного пользователя; передавать profileId в запросе не нужно. Пакет не подключает SDK, не вычисляет эксперименты и не отправляет второй клик. User ID и назначения определяет сервер.
